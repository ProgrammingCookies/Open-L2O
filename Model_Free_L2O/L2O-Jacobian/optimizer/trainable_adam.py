...
class TrainableAdam(opt.TrainableOptimizer):
    ...
    def __init__(self,
        ...
        self.log_learning_rate = tf.Variable(
            np.log(learning_rate), name="log_learning_rate", dtype=tf.float32)
        self.beta1_logit = tf.Variable(
            inv_sigmoid(beta1), name="beta1_logit", dtype=tf.float32)
        self.beta2_logit = tf.Variable(
            inv_sigmoid(beta2), name="beta2_logit", dtype=tf.float32)
        self.log_epsilon = tf.Variable(
            np.log(epsilon), name="log_epsilon", dtype=tf.float32)

        # Key names are derived from Algorithm 1 described in
        # https://arxiv.org/pdf/1412.6980.pdf
        state_keys = ["m", "v", "t"]
        super().__init__("Adam", state_keys, **kwargs)

    @property
    def trainable_variables(self):
        return [self.log_learning_rate, self.beta1_logit, self.beta2_logit,
                self.log_epsilon]
    ...
    def _compute_update(self, param, grad, state):
        ...
        # NOTE: the original (pre-migration) code called _debias_adam_estimate
        # here instead of _update_adam_estimate -- an apparent copy-paste bug
        # (mismatched argument order: beta2 landed in the "t_step" slot and
        # grad_values**2 landed in the "beta" slot), which would have made the
        # second-moment estimate not track squared gradients at all. Fixed to
        # match the same EMA-update pattern used for the first moment above,
        # and Algorithm 1 in the cited Adam paper.
        new_second_moment = self._update_adam_estimate(
