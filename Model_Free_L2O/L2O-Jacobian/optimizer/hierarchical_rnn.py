...
class HierarchicalRNN(opt.TrainableOptimizer):
    ...
    def __init__(self, level_sizes, init_lr_range=(1e-6, 1e-2),
                 ...
                 random_seed=None,
                 biasgrucell_scale=0.5, biasgrucell_gate_bias_init=2.2,
                 hrnn_rnn_readout_scale=0.5, hrnn_default_decay_var_init=2.2,
                 scale_decay_bias_init=3.2, learning_rate_momentum_logit_init=3.2,
                 hrnn_affine_scale=0.5,
                 random_sparse_method="layer_wise", random_sparse_prob=None,
                 **kwargs):
        ...
        Args:
          ...
          biasgrucell_scale: initialization scale for the internal BiasGRUCell vars.
          biasgrucell_gate_bias_init: bias for the internal BiasGRUCell reset and
              update gate variables.
          hrnn_rnn_readout_scale: initialization scale for the RNN readouts.
          hrnn_default_decay_var_init: default initializer value for any
              decay/momentum style variables and constants (sigmoid(2.2) ~ 0.9).
          scale_decay_bias_init: initialization for the scale decay bias -- the
              initial timescale for the exponential avg of the mean square gradients.
          learning_rate_momentum_logit_init: initialization for the learning rate
              momentum.
          hrnn_affine_scale: initialization scale for the weight matrix of the
              bias variables in layer0 and 1 of the hrnn.
          random_sparse_method: HALO's hardware-aware sparse-update mode, one
              of 'layer_wise' (whole-tensor coin flip), 'params_wise'
              (independent per-element coin flip), or 'fix_num' (exact top-k
              selection). See _compute_updates. (Default: 'layer_wise')
          random_sparse_prob: per-tensor keep-probability/keep-fraction list,
              cycled across parameter tensors. [1.0] (the default) is a
              no-op -- every parameter is always updated.
        ...
        # Per-parameter gradient-keep flags from problem.gradients()'s own
        # (optimizer-agnostic) sparse-update masking -- see set_grad_flag_list.
        # None (the default) means "not gated by Layer A", matching the
        # original where this is only ever wired in by metaopt.py's
        # adaptation-evaluation test_optimizer, never during from-scratch
        # meta-training.
        self.grad_flag_list = None

        self.random_seed = random_seed
        self._hrnn_affine_scale = hrnn_affine_scale
        ...
        self._initialize_rnn_cells(level_sizes, biasgrucell_scale,
                                   biasgrucell_gate_bias_init)
        ...
        # BiasGRUCell call.
        scale_factor = hrnn_rnn_readout_scale / math.sqrt(cell_size)
        ...
        self.update_weights = tf.Variable(
            scaled_init((cell_size, 1)), name="update_weights")
        ...
        if self.use_attention:
            ...
            self.attention_weights = tf.Variable(
                self.update_weights, name="attention_weights")
        else:
            self.attention_weights = None

        # weights for projecting the hidden state to the RMS decay term
        self._initialize_scale_decay((cell_size, 1), scaled_init, scale_decay_bias_init)
        self._initialize_input_decay((cell_size, 1), scaled_init, hrnn_default_decay_var_init)

        self._initialize_lr((cell_size, 1), scaled_init)

        # Affine sub-layers used inside the per-parameter loop of
        # _compute_updates. Like lr_momentum_logit/param_stepsize_offset
        # below, these must be created once here (not per-parameter) to
        # preserve the original TF1 code's "one shared variable, reused via
        # variable_scope(reuse=...)" behavior. Their input widths are all
        # known analytically from the level_sizes/num_gradient_scales config,
        # so they can be built eagerly rather than relying on Keras's lazy
        # build-on-first-call.
        self._param_affine = None
        self._global_affine = None
        self._layer_affine = None
        if self.num_layers > 1:
            sz0 = 3 * self.cells[0].state_size
            self._param_affine = utils.AffineLayer(
                sz0, scale=hrnn_affine_scale, random_seed=self.random_seed,
                name="Param_Affine")
            if self.num_layers == 3:
                self._global_affine = utils.AffineLayer(
                    sz0, scale=hrnn_affine_scale, random_seed=self.random_seed,
                    name="Global_Affine")
                sz1 = 3 * self.cells[1].state_size
                self._layer_affine = utils.AffineLayer(
                    sz1, scale=hrnn_affine_scale, random_seed=self.random_seed,
                    name="Layer_Affine")
            # Force these to build now, using the known input widths, rather
            # than lazily on first call.
            self._param_affine([tf.zeros([1, level_sizes[1]])])
            if self.num_layers == 3:
                self._global_affine([tf.zeros([1, level_sizes[2]])])
                self._layer_affine([tf.zeros([1, level_sizes[2]])])

        self._grads_to_delta_affine = None
        self._grads_to_attn_delta_affine = None
        if use_gradient_shortcut:
            grads_width = self.num_gradient_scales
            self._grads_to_delta_affine = utils.AffineLayer(
                1, include_bias=False, vec_mean=1. / grads_width,
                random_seed=self.random_seed, name="GradsToDelta")
            self._grads_to_delta_affine(tf.zeros([1, grads_width]))
            if use_attention:
                self._grads_to_attn_delta_affine = utils.AffineLayer(
                    1, include_bias=False, vec_mean=1. / grads_width,
                    random_seed=self.random_seed, name="GradsToAttnDelta")
                self._grads_to_attn_delta_affine(tf.zeros([1, grads_width]))

        # Shared across all parameters in the per-param loop of
        # _compute_updates -- in the original TF1 code these were created via
        # tf.get_variable inside that loop under a variable_scope(reuse=...),
        # meaning exactly one variable of each was ever created (reused for
        # every subsequent parameter). Creating them here in __init__ instead
        # preserves that "one shared variable, not one per parameter" behavior.
        self.lr_momentum_logit = tf.Variable(
            learning_rate_momentum_logit_init, name="learning_rate_momentum_logit")
        self.param_stepsize_offset = tf.Variable(-1., name="param_stepsize_offset")
    ...
    def set_grad_flag_list(self, grad_flag_list):
        """Shares problem.gradients()'s per-tensor keep/skip decision (Layer
        A) with this optimizer's own sparse-update mechanism (Layer B, see
        _compute_updates), so a learned optimizer and baseline optimizers can
        be compared under the same per-step sparsity pattern during
        adaptation evaluation. Called by metaopt.py's test_optimizer; never
        called during from-scratch meta-training.
        """
        self.grad_flag_list = grad_flag_list

    @property
    def trainable_variables(self):
        variables = [self.update_weights, self.lr_momentum_logit,
                    self.param_stepsize_offset]
        if self.attention_weights is not None:
            variables.append(self.attention_weights)
        for v in (self.scl_decay_weights, self.scl_decay_bias,
                  self.inp_decay_weights, self.inp_decay_bias,
                  self.lr_weights, self.lr_bias):
            if isinstance(v, tf.Variable):
                variables.append(v)
        for cell in self.cells:
            variables.extend(cell.trainable_variables)
        for v in self.init_vectors:
            if isinstance(v, tf.Variable):
                variables.append(v)
        for layer in (self._param_affine, self._global_affine, self._layer_affine,
                     self._grads_to_delta_affine, self._grads_to_attn_delta_affine):
            if layer is not None:
                variables.extend(layer.trainable_variables)
        return variables

    def _initialize_rnn_cells(self, level_sizes, biasgrucell_scale,
                              biasgrucell_gate_bias_init):
        ...
        for level in range(self.num_layers):
            hcell = rnn_cells.BiasGRUCell(
                level_sizes[level],
                scale=biasgrucell_scale,
                gate_bias_init=biasgrucell_gate_bias_init,
    ...
    def _initialize_scale_decay(self, weights_tensor_shape, scaled_init,
        ...
        Args:
          ...
          scale_decay_bias_init: initial value for the scale decay bias.
        """
        if self.learnable_decay:
            self.scl_decay_weights = tf.Variable(
                scaled_init(weights_tensor_shape), name="scl_decay_weights")
            self.scl_decay_bias = tf.Variable(
                tf.fill((1,), scale_decay_bias_init), name="scl_decay_bias")
        else:
            self.scl_decay_weights = tf.zeros_like(self.update_weights)
            self.scl_decay_bias = tf.math.log(0.93 / (1. - 0.93))

    def _initialize_input_decay(self, weights_tensor_shape, scaled_init,
                                hrnn_default_decay_var_init):
        """Initializes the input scale decay weights and bias variables or tensors.

        Args:
          weights_tensor_shape: The shape the weight tensor should take.
          scaled_init: The scaled initialization for the weights tensor.
          hrnn_default_decay_var_init: default decay bias init.
        """
        if (self.learnable_decay and self.num_gradient_scales > 1 and
                self.learnable_inp_decay):
            self.inp_decay_weights = tf.Variable(
                scaled_init(weights_tensor_shape), name="inp_decay_weights")
            self.inp_decay_bias = tf.Variable(
                tf.fill((1,), hrnn_default_decay_var_init), name="inp_decay_bias")
        else:
            self.inp_decay_weights = tf.zeros_like(self.update_weights)
            self.inp_decay_bias = tf.math.log(0.89 / (1. - 0.89))
    ...
    def _initialize_state(self, var):
        ...
        ndim = var_vectorized.shape.as_list()[0]
        ...
        if self.dynamic_output_scale:
            ...
            if min_lr == max_lr:
                log_init_lr = tf.math.log(min_lr * tf.ones_like(var_vectorized))
            else:
                ...
                actual_vals = tf.random.uniform(var_vectorized.shape.as_list(),
                                                np.log(min_lr) / 2.,
                                                np.log(max_lr) / 2.,
                                                seed=self.random_seed)
    ...
    def _compute_updates(self, params, grads, states, global_state):
        ...
        if self.random_sparse_method not in ("layer_wise", None):
            raise NotImplementedError(
                "HierarchicalRNN's sparse-update port only implements "
                "random_sparse_method='layer_wise' so far -- 'params_wise' and "
                "'fix_num' require restructuring _compute_updates to gather/"
                "scatter per-element sub-vectors through the RNN cell, which "
                "hasn't been ported (see migration_changes.txt).")

        layer_state = None
        # Iterate over the layers. HALO's hardware-aware sparse update
        # ('layer_wise' mode): each tensor either gets a full RNN-driven
        # update this step, or is skipped entirely (old param/state passed
        # through unchanged) -- a coarser, whole-tensor version of the same
        # idea as problem.gradients()'s per-tensor gradient masking (Layer
        # A). When self.grad_flag_list has been set (only ever done by
        # metaopt.py's test_optimizer, for comparing a learned optimizer
        # against baseline optimizers under the same sparsity pattern),
        # that shared per-tensor decision is reused directly instead of a
        # fresh draw -- keeping the optimizer's own skip-decision consistent
        # with whatever Layer A already decided for that tensor this step.
        # During meta-training, self.grad_flag_list is never set (see
        # HierarchicalRNN.__init__), so this always draws its own mask from
        # random_sparse_prob, matching the original's from-scratch
        # meta-training behavior. The original's tf.cond(shape==0, ...)
        # becomes a plain Python `if` here -- eager mode already knows the
        # concrete boolean, no graph branch tracing needed.
        for i, (param, grad_unflat, state) in enumerate(zip(params, grads, states)):
            rd_ratio = self.random_sparse_prob[i % len(self.random_sparse_prob)]
            if self.grad_flag_list is not None:
                keep = bool(self.grad_flag_list[i])
            else:
                keep = bool(tf.random.uniform(shape=[], maxval=1) <= rd_ratio)

            if not keep:
                updated_params.append(param)
                updated_attention.append(param)
            ...
            for i2 in range(self.num_gradient_scales):
                new_state["grad_accum{}".format(i2 + 1)] = grads_accum[i2]
                new_state["ms{}".format(i2 + 1)] = mean_squared_gradients[i2]
    ...
    def _compute_mean_log_lr(self, states):
        ...
        if self.use_problem_lr_mean and self.use_relative_lr:
            ...
            for state in states:
                ...
                count_log_lr += state["log_learning_rate"].shape.num_elements()
    ...
    def _update_rnn_cells(self, state, global_state, rnn_input_tensor,
        ...
        total_bias = None
        if self.num_layers > 1:
            param_bias = self._param_affine([state["layer"]])
            total_bias = param_bias
            if self.num_layers == 3:
                global_bias = self._global_affine(global_state)
        ...
        if self.num_layers > 1:
            ...
            if not use_additional_features:
                ...
                layer_input = tf.reduce_mean(new_param_state, 0, keepdims=True)
            else:
                layer_input = tf.reduce_mean(
                    tf.concat((new_param_state, rnn_input_tensor), 1), 0,
                    keepdims=True)
            if self.num_layers == 3:
                layer_bias = self._layer_affine(global_state)
    ...
    def _compute_rnn_state_projections(self, state, new_param_state,
        ...
        if self.use_gradient_shortcut:
            ...
            update_delta += self._grads_to_delta_affine(grads_scaled_tensor)
        if self.dynamic_output_scale:
        ...
        if self.use_attention:
            ...
            attention_delta = utils.project(new_param_state,
                                            attention_weights)
            if self.use_gradient_shortcut:
                attention_delta += self._grads_to_attn_delta_affine(
                    grads_scaled_tensor)
            if self.dynamic_output_scale:
    ...
    def _compute_new_learning_rate(self, state, new_param_state):
        if self.dynamic_output_scale:
            ...
            lrm = tf.nn.sigmoid(self.lr_momentum_logit)
            new_log_lr = (lrm * state["log_learning_rate"] +
                         (1. - lrm) * step_log_lr)
            lr_param = tf.exp(step_log_lr + self.param_stepsize_offset)
