...
This module is a fork of metaopt.py (mirroring the original's own copy-paste
fork), differing only in test_optimizer's adaptation-evaluation loop, which
records test-set accuracy at finer within-epoch intervals (record_num times
per epoch) instead of once per epoch -- suited to tracking how quickly a
resource-constrained device recovers accuracy during continuous, on-device
adaptation, rather than an accuracy curve at full-epoch resolution.
    ...
    for problem_itr, (problem_spec, dataset, batch_size) in enumerate(sampler):
        ...
        for k in range(num_meta_iterations):
            ...
            for unroll_itr in range(num_unrolls):
                ...
                # Whether reg_optimizer's regularization term contributes for
                # this whole unroll segment. 'posterior': on for the later
                # part of the unroll (after reg_scale fraction of segments);
                # 'prior': on for the earlier part; 'none': always off; any
                # other value: always on -- matches the original exactly.
                if regularize_time == "posterior":
                    jacob_switch = unroll_itr > int(num_unrolls * reg_scale + 1)
                elif regularize_time == "prior":
                    jacob_switch = unroll_itr < int(num_unrolls * reg_scale + 1)
                elif regularize_time == "none":
                    jacob_switch = False
                else:
                    jacob_switch = True
                ...
                with tf.GradientTape() as meta_tape:
                    ...
                    meta_obj, sub_obj = opt.partial_unroll(
                        partial_unroll_iters[unroll_itr], objective_weights[unroll_itr],
                        dataset_batches[unroll_itr], jacob_switch=jacob_switch)
...
def test_optimizer(optimizer,
    ...
    Args:
      ...
      num_iter: The number of iterations of the optimizer to run. Ignored if
        test_dataset is given (num_epochs takes over the outer loop then).
      ...
      pretrained_model_path: HALO warm-start -- path to a pickled dict of
        numpy arrays (see train_pretrain.py), forwarded to
        problem.init_variables(). None (the default) initializes randomly.
      test_dataset: a held-out Dataset to periodically measure accuracy
        against -- HALO's "warm-start + continue training + periodically
        test" adaptation-evaluation workflow. If given, switches to an
        epoch-based train+test loop (num_epochs required) instead of the flat
        num_iter one above, and the return value changes -- see Returns.
      num_epochs: number of passes over `dataset` to run when test_dataset is
        given.
      record_num: number of accuracy measurements to take within each epoch
        (this module's one difference from metaopt.py's test_optimizer,
        which only measures once per epoch) -- suited to tracking accuracy
        recovery at finer time resolution during continuous on-device
        adaptation. Default 100.

    Returns:
      If test_dataset is None (the original, non-HALO behavior): unchanged --
        (objective_values, parameters, records).
      If test_dataset is given: (objective_values, acc_records, parameters) --
        acc_records is the list of test-set accuracies measured over the run.
    """
    if dataset is None:
        dataset = datasets.EMPTY_DATASET
        batch_size = dataset.size
    ...
    # Layer A/Layer B coordination for HALO's sparse-update evaluation (see
    # optimizer/hierarchical_rnn.py and problems/problem_generator.py): only
    # HierarchicalRNN implements set_grad_flag_list, baseline
    # tf.keras.optimizers never do -- guarded rather than assumed, matching
    # the original's try/except around the same call.
    has_grad_flag_hook = hasattr(optimizer, "set_grad_flag_list")

    if test_dataset is not None:
        assert num_epochs is not None, "num_epochs is required when test_dataset is given"
        steps_per_epoch = max(1, dataset.data.shape[0] // batch_size)
        record_length = max(1, steps_per_epoch // record_num)

        objective_values = []
        acc_records = []
        step = 0
        for epoch in range(num_epochs):
            for batch in dataset.batch_indices(steps_per_epoch, batch_size):
                ...
                if has_grad_flag_hook:
                    optimizer.set_grad_flag_list(problem.last_grad_flag_list)
                optimizer.apply_gradients(zip(gradients, params))
                objective_values.append(float(obj))

                step += 1
                if step % record_length == 0:
                    acc = float(problem.accuracy(params, test_dataset.data, test_dataset.labels))
                    acc_records.append(acc)
                    print("epoch {} step {} test accuracy {:.4f}".format(epoch, step, acc),
                         flush=True)

        parameters = [p.numpy() if hasattr(p, "numpy") else np.asarray(p) for p in params]
        return objective_values, acc_records, parameters
