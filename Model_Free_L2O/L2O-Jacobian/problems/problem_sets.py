...
Note: adapter_rosenbrock_local/worker (which wrapped problem_generator's
make_rosenbrock_loss_and_init through model_adapter.ModelAdapter) were
dropped -- model_adapter.py monkey-patches tf.Variable internals to intercept
graph-mode variable creation, which has no eager-mode equivalent, and
Rosenbrock is already available natively via pg.Rosenbrock.
"""
...
# ------------------------------------------------------------------
# HALO (Model_Free_L2O/L2O-Jacobian): hardware-aware sparse-update
# meta-training and class-incremental adaptation problem sets. See
# migration_changes.txt for the affine_size derivations (VALID-padding conv
# + maxpool-per-layer shrinks spatial dims fast; HALO's own
# adapt_cifar10_conv_problems_v3, a 4-conv-layer variant, does not fit this
# architecture at all -- spatial dims collapse to zero by the 4th layer --
# and was dropped in favor of the 2-layer adapt_cifar10_conv_problems for
# evaluate_wearable.py, since making a 4-layer variant fit would require
# restructuring ConvNet itself, out of scope for a HALO feature port).
# ------------------------------------------------------------------

def pretrain_mnist_conv_problems(batch_size=64):
    """10-class MNIST ConvNet, the pretrained checkpoint adapt_mnist_conv_problems warm-starts from."""
    return [
        (_Spec(pg.ConvNet, ((1, 28, 28), 10, [(3, 3, 8), (5, 5, 8)]),
               {"activation": tf.nn.relu, "affine_size": 128}), datasets.mnist(train=True), batch_size)
    ]


def adapt_mnist_conv_problems(use_aux_convex=False, aux_convex_dim=20, aux_convex_ratio=1.0,
                              batch_size=64, random_sparse_method=None, random_sparse_prob=None):
    """5-class (odd-digit) MNIST adaptation task."""
    return [
        (_Spec(pg.ConvNet, ((1, 28, 28), 5, [(3, 3, 8), (5, 5, 8)]),
               {"activation": tf.nn.relu, "affine_size": 128,
                "use_aux_convex": use_aux_convex, "aux_convex_dim": aux_convex_dim,
                "aux_convex_ratio": aux_convex_ratio, "random_sparse_method": random_sparse_method,
                "random_sparse_prob": random_sparse_prob}),
         datasets.mnist(train=True, dataset_type="A"), batch_size)
    ]


def adapt_mnist_conv_problems_wide(use_aux_convex=False, aux_convex_dim=20, aux_convex_ratio=1.0,
                                   batch_size=64, random_sparse_method=None, random_sparse_prob=None):
    """Wider (16-channel) variant of adapt_mnist_conv_problems."""
    return [
        (_Spec(pg.ConvNet, ((1, 28, 28), 5, [(3, 3, 16), (5, 5, 16)]),
               {"activation": tf.nn.relu, "affine_size": 256,
                "use_aux_convex": use_aux_convex, "aux_convex_dim": aux_convex_dim,
                "aux_convex_ratio": aux_convex_ratio, "random_sparse_method": random_sparse_method,
                "random_sparse_prob": random_sparse_prob}),
         datasets.mnist(train=True, dataset_type="A"), batch_size)
...
def adapt_cifar10_conv_problems(use_aux_convex=False, aux_convex_dim=20, aux_convex_ratio=1.0,
                                batch_size=64, random_sparse_method=None, random_sparse_prob=None):
    """5-class (odd-class) CIFAR-10 adaptation task."""
    return [
        (_Spec(pg.ConvNet, ((3, 32, 32), 5, [(3, 3, 16), (5, 5, 16)]),
               {"activation": tf.nn.relu, "affine_size": 400,
                "use_aux_convex": use_aux_convex, "aux_convex_dim": aux_convex_dim,
                "aux_convex_ratio": aux_convex_ratio, "random_sparse_method": random_sparse_method,
                "random_sparse_prob": random_sparse_prob}),
         datasets.cifar10(train=True, dataset_type="A"), batch_size)
    ]
