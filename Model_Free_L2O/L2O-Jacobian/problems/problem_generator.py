...
Two "build" entry points are kept deliberately separate (rather than unified
into a single (x_vars, const_vars, loss_fn) tuple, as in the DM/RNNProp
migration): `init_tensors()` returns plain tensors for the meta-training path
(TrainableOptimizer.partial_unroll needs to backprop through many unrolled
steps without tf.Variable indirection), and `init_variables()` returns real
tf.Variables for the standalone drop-in-optimizer path
(TrainableOptimizer.apply_gradients / metaopt.test_optimizer). Both call
objective()/gradients() the same way.
...
def expand_list(_len, x):
    """Distributes x's entries evenly across _len slots, repeating the last
    entry to fill any remainder (see HierarchicalRNN's random_sparse_prob
    cycling for the analogous per-optimizer-tensor version of this idea).
    """
...
class Problem:
    ...
    def __init__(self, param_shapes, random_seed, noise_stdev, init_fn=None,
                random_sparse_method=None, random_sparse_prob=None):
        ...
        Args:
          ...
          init_fn: Unused (kept for signature compatibility with the original;
            was a tf.Session-based variable initializer, which has no meaning
            in eager mode).
          random_sparse_method: HALO hardware-aware evaluation setting -- see
            `gradients()`. None (the default, used by every problem outside
            the HALO adaptation-evaluation scripts) disables it entirely.
          random_sparse_prob: per-tensor keep-probability list for the above;
            [1.0] (the default) is a no-op (every draw is kept).
        ...
        # Pick a random seed.
        self.random_seed = (np.random.randint(MAX_SEED, dtype=np.int64) if random_seed is None
                            else random_seed)
        ...
        self.random_sparse_prob = random_sparse_prob if random_sparse_prob is not None else [1.0]
    ...
    def init_variables(self, seed=None):
        ...
        return [tf.Variable(param) for param in self.init_tensors(seed)]
    ...
    def gradients(self, objective, parameters, tape):
        """Compute gradients of the objective with respect to the parameters.

        Args:
          objective: The objective tensor (e.g. output of self.objective())
          parameters: A list of tensors (the parameters to optimize)
          tape: The tf.GradientTape that was active while `objective` was computed
              (and which watched `parameters`).

        Returns:
          A list of tensors representing the gradient for each parameter,
            returned in the same order as the given list

        Side effect: sets self.last_grad_flag_list, a list of Python bools
        (one per parameter) recording which parameters' gradients were kept
        vs. zeroed by the HALO random_sparse_prob masking below (used by
        metaopt.py's test_optimizer to also gate a HierarchicalRNN's own
        per-tensor update-skipping via optimizer.set_grad_flag_list -- see
        optimizer/hierarchical_rnn.py). Every value is True when
        random_sparse_prob is left at its default [1.0].
        """
        parameters_list = list(parameters)
        grads = tape.gradient(objective, parameters_list)

        # HALO's hardware-aware gradient masking: independently, per
        # parameter tensor, zero the gradient with some probability before it
        # ever reaches the optimizer -- this is what lets baseline optimizers
        # (SGD/Adam/Adagrad), which have no update-skipping logic of their
        # own, be compared on equal footing against HierarchicalRNN's own
        # sparse-update mechanism (see hierarchical_rnn.py). The original's
        # tf.cond(grad_flag, real_gradient, fake_gradient) becomes a plain
        # Python `if` here because the branch is decided outside any traced
        # graph -- same category of change as every other tf.cond removed
        # during the L2O-Scale migration. Note: unlike the original's graph,
        # this doesn't skip computing the "real" gradient itself (tape.gradient
        # above already computed all of them at once) -- only the *result* is
        # masked. Observable gradient values are identical either way; only
        # the original's per-tensor backward-pass compute saving isn't
        # reproduced, which doesn't affect correctness.
        expand_random_sparse_prob = expand_list(len(parameters_list), self.random_sparse_prob)
        grad_flag_list = []
        masked_grads = []
        for grad, rd_ratio in zip(grads, expand_random_sparse_prob):
            grad_flag = bool(tf.random.uniform(shape=[], maxval=1) <= rd_ratio)
            grad_flag_list.append(grad_flag)
            if grad_flag:
                masked_grads.append(grad)
            elif isinstance(grad, tf.IndexedSlices):
                masked_grads.append(tf.IndexedSlices(tf.zeros_like(grad.values), grad.indices))
            else:
                masked_grads.append(tf.zeros_like(grad))
        self.last_grad_flag_list = grad_flag_list

        noisy_grads = []
        for grad in masked_grads:
            if isinstance(grad, tf.IndexedSlices):
                ...
                new_grad = grad + self.noise_stdev * tf.random.normal(grad.shape)
            noisy_grads.append(new_grad)

        return noisy_grads

    # ------------------------------------------------------------------
    # Hessian/Jacobian-based flatness regularization (see optimizer's
    # reg_optimizer/reg_optimizee). Ported from the original's
    # tf.gradients(grads, params, grad_ys=v) Hessian-vector-product (HVP)
    # pattern -- a Python for-loop of graph ops calling the classic
    # Pearlmutter double-backprop trick. In eager mode the same trick needs
    # a *persistent* outer tape wrapping a plain inner tape: the inner tape
    # computes grads = d(objective)/d(params) once, and the (still-open,
    # persistent) outer tape is then queried repeatedly -- once per
    # Hutchinson/power-iteration/Lanczos step, each with a different random
    # probe vector v -- for Hv = d(sum(grads . v))/d(params). This must
    # happen while the outer tape's `with` block is still active, since an
    # op built outside a tape's context is never recorded by it and would
    # come back with a None gradient.
    # ------------------------------------------------------------------

    def jacob_trace(self, grads):
        """Cheap surrogate regularizer: mean of squared gradients."""
        flat = [tf.reshape(tf.square(grad), [1, -1]) for grad in grads]
        return tf.reduce_mean(tf.concat(flat, axis=1))

    def group_add(self, params, update, alpha=1):
        """params = params + update * alpha (list of tensors)."""
        for i, p in enumerate(params):
            params[i] = p + update[i] * alpha
        return params
    ...
    def normalization(self, v):
        """Normalizes a list of tensors (as one flattened vector)."""
        s = tf.sqrt(self.group_product(v, v))
        return [vi / (s + 1e-6) for vi in v]

    def orthnormal(self, w, v_list):
        """Makes w orthogonal to every vector in v_list, then normalizes it."""
        for v in v_list:
            w = self.group_add(w, v, alpha=-self.group_product(w, v))
        return self.normalization(w)

    def _rademacher_like(self, params):
        """A list of tensors matching params' shapes, entries in {-1, +1}."""
        v = [np.random.randint(0, 2, tuple(p.shape.as_list())) for p in params]
        for v_i in v:
            v_i[v_i == 0] = -1
        return [tf.constant(v_i, tf.float32) for v_i in v]

    def hessian_trace(self, params, grads, hess_tape, max_iter=10, tol=1e-3):
        """Trace of the Hessian via Hutchinson's stochastic estimator."""
        del tol  # unused in the original too (early-stopping was never wired in)
        trace_vhv = []
        for _ in range(max_iter):
            v = self._rademacher_like(params)
            grads_dot_v = sum(tf.reduce_sum(g * vi) for g, vi in zip(grads, v))
            Hv = hess_tape.gradient(grads_dot_v, params)
            trace_vhv.append(sum(tf.reduce_sum(hv * vi) for hv, vi in zip(Hv, v)))
        return tf.reduce_mean(tf.stack(trace_vhv))

    def hessian_eigenvalues(self, params, grads, hess_tape, max_iter=10, tol=1e-3,
                            top_n=1):
        """Top-n Hessian eigenvalues via power iteration."""
        del tol  # unused in the original too
        assert top_n >= 1
        ...
        while computed_dim < top_n:
            eigenvalue = None
            v = self.normalization([tf.random.normal(p.shape) for p in params])

            for _ in range(max_iter):
                v = self.orthnormal(v, eigenvectors)
                grads_dot_v = sum(tf.reduce_sum(g * vi) for g, vi in zip(grads, v))
                Hv = hess_tape.gradient(grads_dot_v, params)
                eigenvalue = self.group_product(Hv, v)
    ...
    def density(self, params, grads, hess_tape, max_iter=10, n_v=1):
        """Estimated Hessian eigenvalue spectral density via Stochastic
        Lanczos Quadrature (SLQ). This is what reg_option='hessian-esd' uses.
        """
        eigen_list_full = []
        for _ in range(n_v):
            v = self.normalization(self._rademacher_like(params))
            ...
            for i in range(max_iter):
                if i == 0:
                    grads_dot_v = sum(tf.reduce_sum(g * vi) for g, vi in zip(grads, v))
                    w_prime = hess_tape.gradient(grads_dot_v, params)
                    alpha = self.group_product(w_prime, v)
                    alpha_list.append(alpha)
                    w = self.group_add(list(w_prime), v, alpha=-alpha)
                else:
                    beta = tf.sqrt(self.group_product(w, w))
                    beta_list.append(beta)
                    # The original's `if beta_list[-1] != 0.:` compared a TF1
                    # graph Tensor with Python `!=`, which -- absent operator
                    # overloading building a real op -- is an object-identity
                    # check that's always True (the "generate a fresh restart
                    # vector" branch below was dead code). Eager mode would
                    # perform a real numeric comparison instead and could
                    # silently start taking the other branch. We implement
                    # the evidently-intended numeric zero-check explicitly.
                    if float(beta) != 0.:
                        v = self.orthnormal(w, v_list)
                    else:
                        w_restart = [tf.random.normal(p.shape) for p in params]
                        v = self.orthnormal(w_restart, v_list)
                    v_list.append(v)
                    ...
                    alpha = self.group_product(w_prime, v)
                    alpha_list.append(alpha)
                    w_tmp = self.group_add(list(w_prime), v, alpha=-alpha)
                    w = self.group_add(w_tmp, v_list[-2], alpha=-beta)

            diag1 = tf.linalg.diag(tf.stack(alpha_list))
            diag2 = tf.linalg.diag(tf.stack(beta_list[:len(alpha_list) - 1]))
            up_diag = tf.pad(diag2, [[1, 0], [0, 1]])
            down_diag = tf.pad(diag2, [[0, 1], [1, 0]])
            T = diag1 + up_diag + down_diag

            eigen_list, _ = tf.linalg.eigh(T)
            eigen_list_full.append(eigen_list)

        return tf.reduce_sum(tf.stack(eigen_list_full))

    def regularizer(self, params, data, labels, reg_option, hessian_itrs=10):
        """Computes the Hessian/Jacobian-based regularization term named by
        reg_option, for use by TrainableOptimizer's reg_optimizer/
        reg_optimizee.

        Args:
          params: the (attended) parameters the objective was computed at.
          data, labels: the minibatch the objective/grads were computed on.
          reg_option: one of 'hessian', 'hessian-ev', 'hessian-esd', 'jacob'.
          hessian_itrs: number of power-iteration/Hutchinson/Lanczos steps.

        Returns:
          A scalar regularization tensor.
        """
        if reg_option == "jacob":
            with tf.GradientTape() as tape:
                tape.watch(params)
                obj = self.objective(params, data, labels)
            grads = tape.gradient(obj, params)
            return self.jacob_trace(grads)

        hess_tape = tf.GradientTape(persistent=True)
        with hess_tape:
            hess_tape.watch(params)
            with tf.GradientTape() as grad_tape:
                grad_tape.watch(params)
                obj = self.objective(params, data, labels)
            grads = grad_tape.gradient(obj, params)

            if reg_option == "hessian-ev":
                reg = self.hessian_eigenvalues(params, grads, hess_tape,
                                               max_iter=hessian_itrs)
            elif reg_option == "hessian":
                reg = self.hessian_trace(params, grads, hess_tape,
                                         max_iter=hessian_itrs)
            elif reg_option == "hessian-esd":
                reg = self.density(params, grads, hess_tape, max_iter=hessian_itrs)
            else:
                raise ValueError("Unknown reg_option: {}".format(reg_option))
        del hess_tape
        return reg
...
class SoftmaxClassifier(Problem):
    ...
    def objective(self, params, data, labels, l2_reg_scale=1e-3, convex_l1_ratio=0.1,
        ...
        Args:
          ...
          l2_reg_scale: scaling factor for the parameter-value l2 regularizer.
          convex_l1_ratio: scale for the aux-convex l1 term relative to the l2 one.
          aux_conv_labels_l1, aux_conv_labels_l2: random target tensors for the
              auxiliary convex regularizer (see ConvNet's use_aux_convex_l1/l2),
              or None to disable each term.
        ...
        objs = tf.reduce_mean(losses) + tf.reduce_mean(l2reg) * l2_reg_scale

        if aux_conv_labels_l2 is not None:
            objs += self.aux_convex_ratio * tf.reduce_mean(
                tf.clip_by_value(tf.square(aux_conv_labels_l2 - params[-1]), 0, 10))
        if aux_conv_labels_l1 is not None:
            objs += self.aux_convex_ratio * convex_l1_ratio * tf.reduce_mean(
                tf.clip_by_value(tf.abs(aux_conv_labels_l1 - params[-1]), 0, 10))
...
class ConvNet(SoftmaxClassifier):
    ...
    def __init__(self,
        ...
        # HALO's auxiliary convex regularization: one extra learnable
        # parameter vector the meta-objective pulls toward a random target
        # (see Problem.objective's aux_conv_labels_l1/l2, gated by
        # TrainableOptimizer's separate use_aux_convex_l1/l2 flags -- this
        # single use_aux_convex flag only decides whether the extra parameter
        # exists at all), riding piggyback on the real (non-convex) network
        # parameters.
        ...
        super().__init__(param_shapes, random_seed, noise_stdev,
                         random_sparse_method=random_sparse_method,
                         random_sparse_prob=random_sparse_prob)

        # One owned BatchNormalization layer per conv layer, created once so
        # its weights persist across calls (the eager replacement for the
        # original's tf.layers.batch_normalization, whose TF1 variable-scope
        # based weight sharing has no meaning outside graph mode). These
        # weights are incidental to the forward pass and are not part of
        # `params` -- they are not meta-optimized, matching the original
        # (batch-norm gamma/beta were never part of param_shapes either).
        self._bn_layers = [tf.keras.layers.BatchNormalization()
                           for _ in range(len(filter_list))]

    def init_tensors(self, seed=None, pretrained_model_path=None):
        """Returns a list of tensors with the given shape.

        If pretrained_model_path is given, warm-starts parameters from a
        pickled dict of numpy arrays produced by train_pretrain.py, skipping
        any array whose last-dim shape is 5 (the adapt task's class count) --
        an exact port of the original's own hardcoded `k.shape[-1] != 5`
        check, which is how it avoids loading the pretrain head's final FC
        layer (a different class count) onto the adapt head.
        """
        if pretrained_model_path is not None:
            init_params = [tf.random.normal(shape, mean=0., stddev=0.01, seed=seed)
                          for shape in self.param_shapes]
            ...
            for k_id, k in enumerate(pretrained_params):
                if k.shape[-1] != 5:
                    # tf.constant, not the raw numpy array: eager ops (e.g.
                    # GradientTape.watch) require every entry in this list to
                    # already be a Tensor, unlike TF1 graph mode where a plain
                    # ndarray mixed into a tensor list was implicitly
                    # converted downstream.
                    init_params[k_id] = tf.constant(k, dtype=tf.float32)
    ...
    def inference(self, params, data):
        ...
        for i in range(0, len(w_conv_list), 2):
            ...
            layer = self._bn_layers[i // 2](layer, training=training)
            layer = self.activation(layer)
            output = tf.nn.max_pool2d(layer, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="VALID")
            conv_input = output
        # Flatten.
        flattened = tf.reshape(conv_input, (-1, self.affine_size))
...
class Problem2D(Problem):
    ...
    def surface(self, n=50, xlim=5, ylim=5):
        ...
        xm, ym = _mesh(xlim, ylim, n)
        x = tf.constant(xm, dtype=tf.float32)
        y = tf.constant(ym, dtype=tf.float32)
        zm = self.objective([[x, y]]).numpy()
        return xm, ym, zm
...
class LogSumExp(Problem2D):
    ...
    def objective(self, params, data=None, labels=None):
        x, y = tf.split(params[0], 2, axis=0)
        obj = tf.math.log(tf.exp(x + 3. * y - 0.1) +
                          tf.exp(x - 3. * y - 0.1) +
                          tf.exp(-x - 0.1) + 1.0)
        return tf.squeeze(obj)
...
class Branin(Problem2D):
    ...
    def init_tensors(self, seed=None):
        """Returns a list of tensors with the given shape."""
        x1 = tf.random.uniform((1,), minval=-5., maxval=10., seed=seed)
        x2 = tf.random.uniform((1,), minval=0., maxval=15., seed=seed)
...
class Rescale(Problem):
    ...
    def __init__(self, problem_spec, scale=10., noise_stdev=0.0):
        ...
        super().__init__(self.param_shapes, random_seed=None,
    ...
    def objective(self, params, data=None, labels=None):
        ...
        return self.problem.objective(params_raw, data, labels)
...
class LogObjective(Problem):
    ...
    def __init__(self, problem_spec):
        ...
        super().__init__(self.param_shapes, random_seed=None, noise_stdev=0.0)

    def objective(self, params, data=None, labels=None):
        problem_obj = self.problem.objective(params, data, labels)
        return tf.math.log(problem_obj + EPSILON) - tf.math.log(EPSILON)
...
class SparseProblem(Problem):
    ...
    def __init__(self,
        ...
        super().__init__(self.param_shapes, random_seed=random_seed,
                         noise_stdev=noise_stdev)
    ...
    def gradients(self, objective, parameters, tape):
        grads = tape.gradient(objective, list(parameters))

        new_grads = []
        for grad in grads:
            mask = tf.greater(self.zero_prob, tf.random.uniform(grad.shape))
            zero_grad = tf.zeros_like(grad, dtype=tf.float32)
            noisy_grad = grad + self.noise_stdev * tf.random.normal(grad.shape)
...
class MatMulAlgorithm(Problem):
    ...
    def __init__(self, n, k):
        ...
        super().__init__(param_shapes, random_seed=None, noise_stdev=0.0)
    ...
    def init_tensors(self, seed=None):
        ...
        def _param_initializer(shape, seed=None):
            ...
            return tf.transpose(tf.math.l2_normalize(tf.transpose(x), 1))
    ...
    def objective(self, parameters, data=None, labels=None):
        ...
        p_inv = tf.matmul(
            tf.linalg.inv(tf.matmul(p_trans, p)), p_trans, name="p_inv")
...
def matmul_problem_sequence(n, k_min, k_max):
    ...
    return [(prob_spec.Spec(MatMulAlgorithm, (n, k), {}), None, None)
           for k in range(k_min, k_max + 1)]


def init_fixed_variables(arrays):
    return [tf.Variable(arr.astype("float32")) for arr in arrays]
