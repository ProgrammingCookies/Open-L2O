...
def random_binary(n_features, n_samples, random_seed=None):
    ...
    Returns:
      dataset: A Dataset namedtuple containing the generated data and labels
    """
    random_seed = (np.random.randint(MAX_SEED, dtype=np.int64) if random_seed is None
                   else random_seed)
    np.random.seed(random_seed)
...
def convert_dataset(dataset, dataset_type, num_classes=10):
    """Splits a num_classes-class Dataset into its odd-class ("adapt") or
    even-class ("pretrain") subset, relabeled to a contiguous 0..N-1 range --
    the class-incremental adaptation scenario HALO's adapt_*/pretrain_*
    problem sets are built around.
    """
    if dataset_type == "adapt":
        wanted = list(range(1, num_classes, 2))
    elif dataset_type == "pretrain":
        wanted = list(range(0, num_classes, 2))
    else:
        raise ValueError("No such dataset type: {}".format(dataset_type))

    idx = np.isin(dataset.labels, wanted)
    data, labels = dataset.data[idx], dataset.labels[idx].copy()
    for new_label, old_label in enumerate(wanted):
        labels[dataset.labels[idx] == old_label] = new_label
    return Dataset(data, labels)


def mnist(train=True, dataset_type="full"):
    """Returns MNIST data as a Dataset (images normalized to [0, 1]).

    Args:
      train: if true use training data, else use testing data
      dataset_type: "full" (default, all 10 classes), "adapt"/"A" (odd
          digits, relabeled 0..4), or "pretrain"/"B" (even digits, relabeled
          0..4) -- see convert_dataset.
    """
    (x_train, y_train), (x_test, y_test) = tf.keras.datasets.mnist.load_data()
    imgs, labels = (x_train, y_train) if train else (x_test, y_test)
    imgs = np.reshape(imgs, [-1, 28, 28, 1]).astype(np.float32) / 255.
    dataset = Dataset(imgs.astype("float32"), labels.astype("int32"))

    if dataset_type in ("A", "adapt"):
        dataset = convert_dataset(dataset, "adapt")
    elif dataset_type in ("B", "pretrain"):
        dataset = convert_dataset(dataset, "pretrain")
    return dataset


def cifar10(train=True, data_path=None, dataset_type="full"):
    """Returns CIFAR-10 data as a Dataset (images normalized to [0, 1]).

    Args:
      train: if true use training data, else use testing data
      data_path: unused (kept for call-site signature compatibility) --
          tf.keras.datasets.cifar10 manages its own local cache.
      dataset_type: "full" (default, all 10 classes), "adapt"/"A" (odd
          classes, relabeled 0..4), or "pretrain"/"B" (even classes,
          relabeled 0..4) -- see convert_dataset.
