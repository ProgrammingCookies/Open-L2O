...
def test_optimizer(optimizer,
    ...
    Args:
      ...
      test_dataset: a held-out Dataset to periodically measure accuracy
        ...
        test" adaptation-evaluation workflow (metatest_adapt.py in the
        original). If given, switches to an epoch-based train+test loop
        (num_epochs required) instead of the flat num_iter one above, and the
        return value changes -- see Returns. One accuracy measurement per
        epoch (metaopt_wearable.py's test_optimizer is a separate, forked
        function with finer within-epoch measurement -- see that file).
    ...
    if test_dataset is not None:
        ...
        for epoch in range(num_epochs):
            ...
            print("epoch {} test accuracy {:.4f}".format(epoch, acc), flush=True)
