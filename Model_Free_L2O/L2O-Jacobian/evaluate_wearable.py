...
Fork of evaluate.py (mirroring the original's own copy-paste fork), using
metaopt_wearable's finer within-epoch accuracy tracking (record_num) instead
of metaopt's once-per-epoch tracking -- see metaopt_wearable.py.
"""
...
def parse_args():
    ...
    # HALO adaptation evaluation: warm-start from a pretrained checkpoint,
    # continue training on the adapt (odd-class) split, periodically measure
    # test accuracy on its held-out counterpart.
    ...
    p.add_argument("--record_num", type=int, default=100)
...
def main():
    ...
    # HALO adaptation-evaluation problems, paired with their held-out
    # (dataset_type="A", train=False) test-split counterpart.
    adapt_fns = {
        "adapt_mnist_conv_problems": (ps.adapt_mnist_conv_problems, datasets.mnist),
        "adapt_mnist_conv_problems_wide": (ps.adapt_mnist_conv_problems_wide, datasets.mnist),
        "adapt_cifar10_conv_problems": (ps.adapt_cifar10_conv_problems, datasets.cifar10),
    }
    adapt_problems_and_data = []
    for flag_name, (builder_fn, dataset_fn) in adapt_fns.items():
        if getattr(FLAGS, flag_name):
            (spec, train_dataset, _), = builder_fn(
                batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method,
                random_sparse_prob=[float(p) for p in FLAGS.random_sparse_prob.split(" ")])
            test_dataset = dataset_fn(train=False, dataset_type="A")
            adapt_problems_and_data.append((spec, train_dataset, test_dataset, FLAGS.batch_size))

    if not problems_and_data and not adapt_problems_and_data:
        raise ValueError("No problems selected -- pass at least one --include_*_problems "
                         "or --adapt_*_problems flag.")
    ...
    for problem_itr, (problem_spec_, train_dataset, test_dataset, batch_size) in enumerate(
            adapt_problems_and_data):
        problem = problem_spec_.build()

        for seed in [6, 12, 18, 24, 30]:
            print("adapt-evaluating problem {} using seed {}".format(problem_itr, seed))

            if FLAGS.test_optimizer == "L2o":
                opt = _build_l2o_optimizer(FLAGS)
                if os.path.exists(checkpoint_path):
                    dummy_params = problem.init_variables(seed)
                    dummy_data = train_dataset.data[:2]
                    dummy_labels = train_dataset.labels[:2]
                    with tf.GradientTape() as tape:
                        dummy_obj = problem.objective(dummy_params, dummy_data, dummy_labels)
                    dummy_grads = problem.gradients(dummy_obj, dummy_params, tape)
                    opt.apply_gradients(zip(dummy_grads, dummy_params))
            ...
            objective_values, acc_records, parameters = metaopt.test_optimizer(
                opt, problem, num_iter=0, dataset=train_dataset, batch_size=batch_size,
                seed=seed, pretrained_model_path=FLAGS.pretrained_model_path,
                test_dataset=test_dataset, num_epochs=FLAGS.num_epochs,
                record_num=FLAGS.record_num)

            out_path = os.path.join(
                FLAGS.save_dir,
                "seed{}_adapt_acc_record.pickle-{}".format(seed, FLAGS.model_name))
            with open(out_path, "wb") as f:
                pickle.dump(acc_records, f)
            print("saved adapt accuracy records for seed {} to {}".format(seed, out_path))
